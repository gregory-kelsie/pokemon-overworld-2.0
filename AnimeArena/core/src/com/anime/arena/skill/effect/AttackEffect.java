...
public class AttackEffect extends StatEffect {
    public AttackEffect(SkillTarget target, StatDirection direction, int amount) {
    ...
    public List<String> use(BattlePokemon skillUser, BattlePokemon enemyPokemon, Field field, SubField userField, SubField enemyField, boolean isFirstAttack) {
        ...
        if (statDirection == StatDirection.DECREASE) {
            ...
            if (receiverField.hasMist()) {
                ...
                addEffectMessage(effectMessages,effectReceiver.getName() + "'s attack can't be lowered.");
            } else {

                effectReceiver.decreaseAttackStage(finalAmount);
                addEffectMessage(effectMessages,effectReceiver.getName() + "'s attack" + getFallText());
            }
        } else {
            //Attempt to increase the attack stage.
            if (effectReceiver.getAttackStage() == 6) {
                addEffectMessage(effectMessages,effectReceiver.getName() + "'s attack can't get higher.");
            } else {
                effectReceiver.increaseAttackStage(finalAmount);
                addEffectMessage(effectMessages,effectReceiver.getName() + "'s attack" + getRoseText());
            }
        }
